...
async function handleFullAddressLookup(address: string, ip: string) {
  ...
  try {
    ...
    if (!parcel) {
      return NextResponse.json({ properties: [], total: 0, message: 'No property found for this address. Please check the spelling or try selecting a specific borough/county.' });
    }
